...
export default function PMSStockReconciliation() {
  const [records, setRecords] = useState<PMSStockRecord[]>([]);
  ...
  const [formData, setFormData] = useState({
    ...
    sales: 0,
    physicalStock: 0,
    pricePerLiter: 617, // Default PMS price
    tankId: '',
    recordedBy: '',
    notes: '',
  });
  ...
  const loadStations = () => {
    const activeStations = getActiveStations().filter(s =>
      s.productsAvailable.includes('PMS')
    );
    setStations(activeStations);
  };

  const loadRecords = () => {
    const stored = localStorage.getItem('pms_stock_records');
    if (stored) {
      setRecords(JSON.parse(stored));
    } else {
      // Sample data from JABI PMS Stock Reconciliation
      const sampleData: PMSStockRecord[] = [
        {
          id: '1',
          date: '2025-03-01',
          stationId: 'jabi-001',
          ...
          waybillQty: 45000,
          transitLoss: 450, // 1% transit loss
          receipts: 44550,
          sales: 38200,
          closingBalance: 38850,
          physicalStock: 38500,
          dailyOverageShortage: -350,
          pricePerLiter: 617,
          varianceValue: -215950,
          tankId: 'PMS-T1',
          recordedBy: 'John Doe',
      ...
      localStorage.setItem('pms_stock_records', JSON.stringify(sampleData));
    }
  };

  const saveRecords = (newRecords: PMSStockRecord[]) => {
    setRecords(newRecords);
    localStorage.setItem('pms_stock_records', JSON.stringify(newRecords));
  };
  ...
  const handleEdit = (record: PMSStockRecord) => {
    setFormData({
      date: record.date,
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `pms-stock-reconciliation-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
    ...
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Daily stock tracking for Premium Motor Spirit (PMS)</p>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          ...
          <p className="text-2xl font-bold text-blue-600">{summary.totalReceipts.toLocaleString()}L</p>
        </div>
        <div className="bg-white p-4 rounded-lg border border-gray-200">
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          ...
          <p className="text-2xl font-bold text-orange-600">{summary.totalTransitLoss.toLocaleString()}L</p>
        </div>
        <div className="bg-white p-4 rounded-lg border border-gray-200">
              ...
              {filteredRecords.length === 0 ? (
                <tr>
                  <td colSpan={13} className="px-4 py-8 text-center text-gray-500">
                    No records found. Click "New Record" to add your first PMS stock record.
                  </td>
                </tr>
              ) : (
                filteredRecords.map(record => (
                  <tr key={record.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-orange-600 text-right">{record.transitLoss.toLocaleString()}</td>
                    <td className="px-4 py-3 text-sm text-blue-600 text-right font-medium">{record.receipts.toLocaleString()}</td>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4">
                {editingId ? 'Edit PMS Stock Record' : 'New PMS Stock Record'}
              </h2>
